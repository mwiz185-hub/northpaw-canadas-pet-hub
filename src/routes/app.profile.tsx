...
function ProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setProfile(prof);
      const { data: pets } = await supabase.from("pets").select("*").eq("owner_id", user.id).order("created_at");
  ...
  function selectPet(summary: PetSummary) {
    ...
    supabase.from("pets").select("*").eq("id", summary.id).maybeSingle().then(({ data }) => {
    ...
    try {
      ...
      if (pet.id) {
        const { data, error } = await supabase
          .from("pets").update(payload).eq("id", pet.id).eq("owner_id", user.id).select().maybeSingle();
        if (error) throw error;
        if (!data) throw new Error("No pet was updated (check permissions)");
        setPet((p) => ({ ...p, id: data.id }));
        setPetList((list) => list.map((s) => s.id === data.id ? { ...s, name: data.name, photos: data.photos } : s));
      } else {
        const { data, error } = await supabase
          .from("pets").insert(payload).select().maybeSingle();
        if (error) throw error;
        if (!data) throw new Error("Failed to create pet");
        setPet((p) => ({ ...p, id: data.id }));
        setPetList((list) => [...list, { id: data.id, name: data.name, species: data.species, photos: data.photos }]);
      }
  ...
  const autoSaveToggles = useCallback(async (updates: Partial<Pick<Pet, "show_in_mating" | "show_in_adoption" | "show_in_marketplace">>) => {
    ...
    toggleTimer.current = setTimeout(async () => {
      try {
        const { error } = await supabase.from("pets").update(updates).eq("id", petId).eq("owner_id", ownerId);
        if (error) throw error;
        setToggleSaved(true);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs capitalize text-muted-foreground">
                {profile?.user_type === "owner" ? "Pet Owner" : profile?.user_type === "shelter" ? "Shelter" : "Pet Store"} · {profile?.city}
              </p>
            </div>
          </div>
