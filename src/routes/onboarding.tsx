...
function Onboarding() {
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col px-6 py-10">
      <div className="self-start text-primary"><Logo size={40} /></div>
      <h1 className="mt-6 text-2xl font-bold">Tell us about you</h1>
      <p className="text-sm text-muted-foreground">A few details to finish setting up your account.</p>

      <form onSubmit={save} className="mt-6 space-y-4">
        <div>
          <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Account type</span>
            ...
            {(["owner", "shelter", "store"] as const).map((t) => (
              <button type="button" key={t} onClick={() => setUserType(t)}
                className={`rounded-xl border px-2 py-3 text-sm font-medium capitalize ${
                  userType === t ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"
                }`}>
        ...
        {userType !== "owner" && (
          <Input label={userType === "shelter" ? "Shelter name" : "Store name"} value={orgName} onChange={setOrgName} required />
        )}
