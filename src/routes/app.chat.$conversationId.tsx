...
type Conv = { id: string; user_a_id: string; user_b_id: string; context_pet_id: string | null; kind: string };
...
function ChatPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (c && user) {
        ...
        if (c.context_pet_id) {
          const { data: pet } = await supabase.from("pets").select("price").eq("id", c.context_pet_id).maybeSingle();
          setPetPrice(pet?.price ?? null);
        }
        const { data: s } = await supabase.from("sales").select("id, sale_price, commission, created_at").eq("conversation_id", conversationId).maybeSingle();
  ...
  async function confirmSale() {
    ...
    const { data, error } = await supabase.from("sales").insert({
      ...
      sale_price: price,
      commission,
    }).select("id, sale_price, commission, created_at").maybeSingle();
    if (error) {
      setSaving(false);
    ...
    <div className="flex h-[calc(100vh-9rem)] flex-col">
      <div className="flex items-center gap-3 border-b border-border bg-card px-4 py-3">
        ...
        {sale ? (
          ...
          <button onClick={openModal} className="rounded-full bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground">
            Mark as purchased
          </button>
        ) : null}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4" onClick={() => !saving && setShowModal(false)}>
          <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Enter the final sale price. A 2% platform commission will be recorded.</p>
            <label className="mt-4 block text-xs font-medium text-muted-foreground">Sale price (USD)</label>
            <input
            ...
            {priceInput && isFinite(parseFloat(priceInput)) && (
              <p className="mt-2 text-xs text-muted-foreground">
                Commission (2%): <span className="font-medium text-foreground">${(parseFloat(priceInput) * 0.02).toFixed(2)}</span>
              </p>
            )}
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => setShowModal(false)} disabled={saving}
                className="rounded-lg px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-muted">Cancel</button>
              <button onClick={confirmSale} disabled={saving || !priceInput}
